...
function interceptStr(str, interceptLength) {
    if (variableTypeDetection.isString(str)) {
        return str.slice(0, interceptLength) + (str.length > interceptLength ? ";slice the first ".concat(interceptLength, " characters") : '');
    }
    return '';
}
...
var BrowserTransport = (function (_super) {
    ...
    BrowserTransport.prototype.imgRequest = function (data, url) {
        var requestFun = function () {
            ...
            img.src = "".concat(url).concat(spliceStr, "data=").concat(encodeURIComponent(safeStringify(data)));
            img = null;
        };
        this.queue.addTask(requestFun);
    };
...
function resourceTransform(target) {
    return {
        ...
        level: Severity.Low,
        time: getTimestamp(),
        name: "".concat(resourceMap[target.localName] || target.localName, "\u52A0\u8F7D\u5931\u8D25")
    };
}
